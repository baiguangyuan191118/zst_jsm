...
public class BannerLayout extends RelativeLayout {
    ...
    private static final int PAGE_HEIGHT = 160;//dp
    private static final int PAGE_MARGIN = 20;//dp
    private static final int PAGER_HMARGIN = 50;//dp
    private static final int PAGER_TMARGIN = 5;//dp
    private static final int PAGER_BMARGIN = 24;//dp
...
//    private int unSelectedIndicatorColor = 0x88888888;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        selectedDrawable = getResources().getDrawable(array.getResourceId(R.styleable.BannerLayoutStyle_selectedIndicatorSrc, R.drawable.lend_banner_pointer_selected));
        unSelectedDrawable = getResources().getDrawable(array.getResourceId(R.styleable.BannerLayoutStyle_unselectedIndicatorSrc, R.drawable.lend_banner_pointer_unselected));
    ...
    public void setViews(final List<View> views) {
        ...
        pager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int arg0) {
                if (isAutoPlay) {
                    playHandler.sendEmptyMessageDelayed(ImageHandler.MSG_UPDATE_IMAGE, ImageHandler.MSG_DELAY);//自动轮播
                } else {
                    playHandler.sendEmptyMessageDelayed(ImageHandler.MSG_PAGE_CHANGED, ImageHandler.MSG_DELAY);//不自动轮播
                }
            }
...
//    }
    ...
    private static class ImageHandler extends Handler {
        ...
        protected ImageHandler(WeakReference<BannerLayout> wk) {
            weakReference = wk;
        }
        ...
        public void handleMessage(Message msg) {
            ...
            BannerLayout mBannerLayout = weakReference.get();
            if (mBannerLayout == null) {
                // Activity已经回收，无需再处理UI了
                return;
            }
            // 检查消息队列并移除未发送的消息，这主要是避免在复杂环境下消息出现重复等问题。
            if (mBannerLayout.playHandler.hasMessages(MSG_UPDATE_IMAGE)) {
                mBannerLayout.playHandler.removeMessages(MSG_UPDATE_IMAGE);
            }
            switch (msg.what) {
                case MSG_UPDATE_IMAGE:
                    currentItem = mBannerLayout.pager.getCurrentItem();
                    currentItem++;
                    // currentItem = currentItem % activity.PAGE_COUNT;
                    mBannerLayout.pager.setCurrentItem(currentItem, true);
                    // 准备下次播放
                    // 准备下次播放
                    if (mBannerLayout.pagerAdapter.getRealCount() >= 2) {
                        mBannerLayout.playHandler.sendEmptyMessageDelayed(
