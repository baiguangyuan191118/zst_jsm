...
public class GestureLockLayout extends RelativeLayout {
    ...
    //手指触摸是，path颜色 #6ecae4
    private int mFingerTouchColor = Color.parseColor("#6ecae4");
    //手指抬起时,密码匹配path颜色
    private int mFingerUpMatchedColor = Color.parseColor("#6ecae4");
    //手指抬起时,密码不匹配path颜色
    private int mFingerUpUnmatchedColor = Color.RED;
    ...
    private void initTypeValue(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs, R.styleable.GestureLockLayoutStyle, defStyleAttr, 0);
        int indexCount = array.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int attr = array.getIndex(i);

            if (R.styleable.GestureLockLayoutStyle_dot_count == attr) {
                //定义布局是n行*n列
                mDotCount = array.getInt(attr, 3);
            }

            if (R.styleable.GestureLockLayoutStyle_touch_path_color == attr) {
                //手指按下颜色
                mFingerTouchColor = array.getColor(attr, Color.parseColor("#6ecae4"));
            }

            if (R.styleable.GestureLockLayoutStyle_matched_path_color == attr) {
                //手指抬起匹配的颜色
                mFingerUpMatchedColor = array.getColor(attr, Color.parseColor("#6ecae4"));
            }

            if (R.styleable.GestureLockLayoutStyle_unmatched_path_color == attr) {
                //手指抬起不匹配的颜色
                mFingerUpUnmatchedColor = array.getColor(attr, Color.RED);
            }
        }
        array.recycle();
    }
    ...
    private void init(Context context) {
        if (mILockView == null) {
            JDLockView jdLockView = new JDLockView(context);
            setLockView(jdLockView);
        }
     ...
     * @param colorString
     */
    public void setBirthdayColor(String colorString) {
        mFingerTouchColor = Color.parseColor(colorString);
        mFingerUpMatchedColor = Color.parseColor(colorString);
    }
}
